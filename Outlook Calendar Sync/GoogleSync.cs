...
namespace Outlook_Calendar_Sync {
    ...
    public sealed class GoogleSync : IDisposable {
        ...
        private readonly string[] m_scopes = { CalendarService.Scope.Calendar, CalendarService.Scope.CalendarEvents };
        private readonly string m_workingDirectory =
            Environment.GetFolderPath( Environment.SpecialFolder.ApplicationData ) +
            "\\OutlookGoogleSync\\.credentials\\Outlook-Google-Sync.json";

        private const string AUTOMATICALLY_CREATED = "To see detailed information for automatically created events like this one, use the official Google Calendar app.";
        ...
        public List<CalendarItem> PullListOfAppointmentsBySyncToken() {
            try
            {
                ...
                try
                {
                     items = PullListOfAppointments( list, ref syncToken );
                }
                catch ( GoogleApiException ex )
                {
                    Log.Write( ex );
                    ...
                    {
                        Log.Write( "Sync token is invalid. Performing a full sync for calendar: " + m_currentCalendar );
                        list.SyncToken = null;
                        items = PullListOfAppointments( list, ref syncToken );
                    }
                }
        ...
        public void UpdateAppointment( CalendarItem ev ) {
            ...
            if ( ev.Body.Contains(AUTOMATICALLY_CREATED ) )
            {
